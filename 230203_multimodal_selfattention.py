...
class FusionModel(LightningModule):
    ...
    def forward(self,text_input_ids, audio_input_ids, **kwargs):
        ...
        text_embed, _ = self.mha_t(text_embed,text_embed,text_embed) 
        text_embed2 = torch.mean(text_embed, dim=1)
        ...
        audio_embed, _ = self.mha_t(audio_embed,audio_embed,audio_embed) 
        audio_embed2 = torch.mean(audio_embed, dim=1)
        
        #print("audio: ",audio_embed.shape)

        
        fuse1 = torch.stack((text_embed2, audio_embed2), dim=1) 
    ...
    def validation_epoch_end(self, outputs):
        ...
        # pprint(df_result)
        # df_result.to_csv(
        #     f'./result/selfattention_{datetime.now().__format__("%m%d_%H%M")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_val.csv')


        # pred_df = pd.DataFrame(pred_dict)
        # pred_df.to_csv(
        #     f'./result/selfattention_{datetime.now().__format__("%m%d_%H%M")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_val_pred.csv')
    ...
    def test_epoch_end(self, outputs):
        ...
        pred_df = pd.DataFrame(pred_dict)
        pred_df.to_csv(
            f'./result/selfattention_{datetime.now().__format__("%m%d_%H%M%S")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_test_pred.csv')
...
if __name__ == '__main__': 
    ...
    parser.add_argument("--text_nh", type=int, default=2)
    parser.add_argument("--audio_nh", type=int, default=2)
