...
class FusionModel(LightningModule):
    def __init__(self, args, config):
        ...
        self.dense2 = nn.Linear(self.hidden_size, self.num_labels)
    ...
    def forward(self,text_input_ids, audio_input_ids, **kwargs):
        ...
        fuse=torch.cat([text_embed,audio_embed],axis=1)
        #print("fusion :", fuse.shape)
        
        logits=self.dense2(self.dense1(fuse))
    ...
    def validation_epoch_end(self, outputs):
        ...
        pprint(df_result)
        # df_result.to_csv(
        #     f'./result/concat_{datetime.now().__format__("%m%d_%H%M")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_val.csv')


        # pred_df = pd.DataFrame(pred_dict)
        # pred_df.to_csv(
        #     f'./result/concat_{datetime.now().__format__("%m%d_%H%M")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_val_pred.csv')
    ...
    def test_epoch_end(self, outputs):
        ...
        pred_df = pd.DataFrame(pred_dict)
        pred_df.to_csv(
            f'./result/concat_{datetime.now().__format__("%m%d_%H%M%S")}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_test_pred.csv')
